...
from infer_lama.saicinpainting.training.modules.ffc import FFCResnetBlock
from infer_lama.saicinpainting.training.modules.pix2pixhd import ResnetBlock
    ...
    for idi in pbar:
        ...
        for idd, forward_rear in enumerate(forward_rears):
            ...
            if idd < 0:
                ...
                midz1, midz2 = midz1.to(devices), midz2.to(devices)
...
def refine_predict(
    ...
    max_scales : int, px_budget : int, devices
    ):
    ...
    for ids, (image, mask) in enumerate(zip(ls_images, ls_masks)):
        ...
        mask[mask >= 1e-8] = 1.0
        mask[mask < 1e-8] = 0.0
        image, mask = move_to_device(image, devices), move_to_device(mask, devices)
        if image_inpainted is not None:
            image_inpainted = move_to_device(image_inpainted, devices)
